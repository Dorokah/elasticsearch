...
public final class RoaringBitmapAggregationBuilder extends ValuesSourceAggregationBuilder.LeafOnly<RoaringBitmapAggregationBuilder> {
    ...
    /**
     * Guards the {@code max_values} request field and the effective limit carried on the result. The
     * aggregation itself shipped under {@link #ROARING_BITMAP_AGGREGATION_ADDED}, so a node can speak
     * that version without knowing about the limit.
     */
    static final TransportVersion ROARING_BITMAP_MAX_VALUES_ADDED = TransportVersion.fromName("roaring_bitmap_max_values_added");
    ...
    public RoaringBitmapAggregationBuilder(StreamInput in) throws IOException {
        super(in);
        maxValues = in.getTransportVersion().supports(ROARING_BITMAP_MAX_VALUES_ADDED) ? in.readOptionalVInt() : null;
    }
    ...
    protected void innerWriteTo(StreamOutput out) throws IOException {
        if (out.getTransportVersion().supports(ROARING_BITMAP_MAX_VALUES_ADDED)) {
