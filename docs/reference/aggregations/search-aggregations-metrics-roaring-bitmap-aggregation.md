...
Each value falls in exactly one range, so each window's bitmap is complete for its own range, and the union of all windows is exactly the set a single request would have returned.
